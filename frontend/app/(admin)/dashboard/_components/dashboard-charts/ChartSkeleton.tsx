...
export default function ChartSkeleton({ title, description }: ChartSkeletonProps) {
  return (
    <div className="relative overflow-hidden rounded-3xl border border-slate-800/60 bg-slate-950/80 shadow-[0_30px_90px_-60px_rgba(15,23,42,1)] backdrop-blur animate-pulse">
      ...
      <div className="relative flex flex-col gap-4 px-6 py-6">
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <Skeleton className="h-10 w-10 rounded-2xl bg-slate-900/60" />
            <div className="space-y-2">
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-slate-500">
                {title}
              </p>
              <Skeleton className="h-3 w-44 bg-slate-900/50" />
            </div>
          </div>
        ...
        <div className="rounded-2xl border border-slate-800/70 bg-slate-900/40 p-4">
          <Skeleton className="h-48 w-full bg-slate-900/60 rounded-2xl" />
        </div>
      </div>
    </div>
  );
