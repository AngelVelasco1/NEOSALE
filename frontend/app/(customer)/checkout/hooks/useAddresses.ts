...
import * as addressApi from '../services/addressesApi';
...
export const useAddresses = () => {
  ...
  const getAddressById = useCallback(async (address_id: number) => {
    if (!userId) throw new Error('Usuario no autenticado');
  ...
  const createAddress = useCallback(async (addressData: addressApi.CreateAddressData) => {
    if (!userId) throw new Error('Usuario no autenticado');
    return addressApi.createAddress(addressData, userId);
  }, [userId]);

  const updateAddress = useCallback(async (address_id: number, updateData: addressApi.UpdateAddressData) => {
    if (!userId) throw new Error('Usuario no autenticado');
    return addressApi.updateAddress(address_id, updateData, userId);
  }, [userId]);

  const deleteAddress = useCallback(async (address_id: number) => {
    if (!userId) throw new Error('Usuario no autenticado');
    return addressApi.deleteAddress(address_id, userId);
  }, [userId]);

  const setDefaultAddress = useCallback(async (address_id: number) => {
    if (!userId) throw new Error('Usuario no autenticado');
