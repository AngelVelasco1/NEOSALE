...
const NavLink: React.FC<NavLinkProps> = ({
  ...
  <Link
    href={href}
    className={`inline-flex h-11 items-center justify-center rounded-xl px-5 text-sm font-medium transition-all duration-300 hover:bg-white/10 hover:text-white font-inter shadow-lg hover:shadow-xl hover:shadow-blue-500/20 ${className}`}
    {...props}
  >
    {children}
  </Link>
...
const IconButton: React.FC<IconButtonProps> = ({
  ...
  const variants = {
    default: `h-11 w-11 hover:bg-white/10 transition-all duration-300 rounded-xl hover:scale-110 shadow-lg hover:shadow-xl hover:shadow-blue-500/20`,
    heart: `h-11 w-11 hover:bg-white/10 hover:text-pink-400 transition-all duration-300 rounded-xl hover:scale-110 shadow-lg hover:shadow-xl hover:shadow-pink-500/20`,
    cart: `h-11 w-11 hover:bg-white/10 hover:text-emerald-400 transition-all duration-300 rounded-xl hover:scale-110 shadow-lg hover:shadow-xl hover:shadow-emerald-500/20`,
  };
...
export const Navbar = () => {
  ...
  return (
    <nav
      className={`w-full py-3 backdrop-blur-xl bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 sticky top-0 z-50 transition-all duration-500 border-b border-white/10 shadow-2xl shadow-slate-950/50 ${isScrolled ? "bg-slate-950/98 shadow-3xl" : "bg-slate-950/95"
        }`}
    >
        ...
        <div className="flex items-center justify-between h-18">
          ...
          <div className="flex items-center gap-3">
            ...
            <Sheet>
              ...
              <SheetContent
                side="left"
                className="w-80 bg-slate-950/95 backdrop-blur-xl border-r border-white/10 p-6"
              >
                <div className="flex flex-col gap-8 mt-6">
                  <Link
                    href="/"
                    className="text-xl font-semibold text-white hover:text-blue-400 transition-colors duration-300 font-montserrat"
                  >
                    Inicio
                  </Link>
                      ...
                      categories.map((category, index) => (
                        <div key={category.id} className="space-y-3">
                          ...
                          <Link
                            ...
                            className="flex items-center gap-3 py-3 px-4 rounded-lg bg-white/5 hover:bg-white/10 transition-colors group border border-white/10"
                          >
                            ...
                            <div className="flex items-center gap-2 flex-1">
                              ...
                              {category.featured && (
                                <span className="text-[10px] px-2 py-1 bg-blue-500/20 text-blue-300 rounded font-medium border border-blue-400/30">
                                  Popular
                                </span>
                              )}
                            </div>
                          ...
                          <div className="pl-8 space-y-1">
                            {category.subcategories.slice(0, 4).map((sub) => (
                              <Link
                                ...
                                className="block text-sm text-gray-300 hover:text-white py-2 px-3 rounded hover:bg-white/5 transition-colors font-medium"
                              >
                                {sub.name}
                              </Link>
                            ))}
                            {category.subcategories.length > 4 && (
                              <Link
                                href={createCategoryUrl(category.name)}
                                className="block text-sm text-blue-400 hover:text-blue-300 py-2 px-3 rounded hover:bg-blue-500/10 transition-colors font-semibold"
                              >
          ...
          <NavigationMenu className="hidden text-white lg:flex flex-1 justify-center max-w-3xl mx-4">
            <NavigationMenuList className="gap-1 [&>li]:!bg-transparent [&>li]:!border-none [&>li]:!shadow-none">
              ...
              <NavigationMenuItem>
                ...
                <NavigationMenuContent className="min-w-0 [&>div]:!bg-transparent [&>div]:!border-none [&>div]:!shadow-none [&>div]:!p-0">
                  ...
                  <div className="w-[760px] bg-slate-950/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl shadow-slate-950/50 p-8">
                    {categoriesLoading ? (
                      ...
                      <div className="flex items-center justify-center py-16">
                        <div className="text-center">
                          ...
                          <p className="text-gray-300 font-medium">No hay categorías disponibles</p>
                        </div>
                      </div>
                    ) : (
                      <div className="grid grid-cols-3 gap-8">
                        {categories.map((category) => (
                          <div key={category.id} className="relative">
                            ...
                            <Link
                              href={createCategoryUrl(category.name)}
                              className="flex items-center gap-3 mb-4 p-3 rounded-lg bg-white/5 hover:bg-white/10 transition-all duration-300 group h-20 border border-white/10 hover:border-blue-400/50 hover:shadow-lg hover:shadow-blue-500/20 hover:-translate-y-0.5"
                            >
                              ...
                              <div className="flex-1 min-w-0 flex flex-col justify-center">
                                ...
                                <h3 className="font-semibold text-white text-sm leading-tight line-clamp-2 group-hover:text-blue-400 transition-colors duration-300 mb-1">
                                  {category.name}
                                </h3>
                                {category.featured && (
                            ...
                            <div className="space-y-2">
                              {category.subcategories.slice(0, 5).map((sub) => (
                                <Link
                                  key={sub.id}
                                  href={createCategoryUrl(category.name, sub.name)}
                                  className=" text-sm text-gray-300 hover:text-white py-2.5 px-3 rounded-lg hover:bg-white/5 transition-all duration-200 font-medium h-10 flex items-center truncate border border-transparent hover:border-white/20 hover:translate-x-1 hover:shadow-sm"
                                >
                                  {sub.name}
                                </Link>
                              ))}
                              {category.subcategories.length > 5 && (
                                <Link
                                  href={createCategoryUrl(category.name)}
                                  className=" text-sm text-blue-400 hover:text-blue-300 py-2.5 px-3 rounded-lg hover:bg-blue-500/10 transition-all duration-200 font-semibold h-10 flex items-center border border-transparent hover:border-blue-400/30 hover:shadow-sm hover:shadow-blue-500/10"
                                >
           ...
           */}
              ...
              {isSearchFocused && searchQuery.length === 0 && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-slate-950/95 backdrop-blur-xl rounded-xl shadow-2xl border border-white/10 p-4 z-50">
                  <div className="text-sm mb-3 font-semibold font-inter text-white">
                    Búsquedas populares:
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {trendingSearches.map((search) => (
                      <button
                        key={search}
                        className="px-4 py-2 bg-white/5 text-white rounded-lg text-sm hover:bg-white/10 hover:text-blue-400 transition-all duration-300 hover:scale-105 font-inter border border-white/10 shadow-sm font-medium"
                        onClick={() => {
          ...
          <div className="text-white flex items-center gap-2">
            ...
            {userProfile ? (
              <DropdownMenu>
                ...
                <DropdownMenuContent
                  align="end"
                  className="w-64 bg-slate-950/95 backdrop-blur-xl shadow-2xl border border-white/10 rounded-xl p-3"
                >
                  ...
                  <DropdownMenuItem
                    ...
                    className="hover:bg-white/10 hover:text-blue-400 cursor-pointer font-inter rounded-lg p-3 font-medium mt-2"
                  >
                  ...
                  <DropdownMenuItem
                    ...
                    className="hover:bg-white/10 hover:text-blue-400 cursor-pointer font-inter rounded-lg p-3 font-medium"
                  >
              ...
              <Button
                ...
                className="h-11 ml-3 px-5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-500 text-white transition-all duration-300 hover:from-blue-700 hover:to-indigo-600 hover:shadow-lg hover:shadow-blue-500/25 hover:scale-105 font-inter shadow-lg ring-1 ring-white/20 font-medium text-sm"
              >
      ...
      <div className="md:hidden px-4 pb-3 mt-2">
        <div className="relative">
          <SearchIcon
            ...
            className="absolute left-4 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400 cursor-pointer hover:text-white transition-colors"
          />
          <Input
            type="search"
            ...
            className="pl-12 pr-4 w-full h-10 rounded-xl transition-all duration-300 font-inter shadow-lg bg-white/5 border-white/10 hover:border-white/20 focus:border-blue-400/50 backdrop-blur-sm text-white placeholder:text-gray-400"
          />

          {isSearchFocused && searchQuery.length === 0 && (
            <div className="absolute top-full left-0 right-0 mt-3 bg-slate-950/95 backdrop-blur-xl rounded-xl shadow-2xl border border-white/10 p-5 z-50">
              <div className="text-sm mb-4 font-semibold font-inter text-white">
                Búsquedas populares:
              </div>
              <div className="flex flex-wrap gap-2">
                {trendingSearches.map((search) => (
                  <button
                    key={search}
                    className="px-4 py-2 bg-white/5 hover:bg-white/10 text-white rounded-lg text-sm hover:text-blue-400 transition-all duration-300 hover:scale-105 font-inter border border-white/10 shadow-sm font-medium"
                    onClick={() => {
                      setSearchQuery(search);
                      setIsSearchFocused(false);
                    }}
